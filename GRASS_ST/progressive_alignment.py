from GRASS_ST.utils.graph_matching_utils import *
import torch
import torch.nn as nn
import torch.nn.functional as F
from torch_geometric.nn import SAGEConv, GraphConv, GCNConv, GINConv
...
class GradAlign:
    def __init__(self, G1, G2, att_s, att_t, alignment_dict, alignment_dict_reversed, 
                 idx1_dict, idx2_dict, 
                 k_hop=2, alpha=10, beta=1, fast_mode=True, 
                 hid=128, seed_ratio=0.6,seed_add_step=10, align_iteration=20,
                 have_embed=True):
        ...
        self.epochs = 5
        self.align_iteration = align_iteration
    ...
    def run_algorithm(self, emb1, emb2):  # anchor is not considered yet
        ...
        if self.seed_ratio == 0:
            ...
            seed_list1 = list(
                np.random.choice(list(self.alignment_dict.keys()), int(self.seed_ratio * len(self.alignment_dict)),
                                 replace=False))
            seed_list2 = [self.alignment_dict[seed_list1[x]] for x in range(len(seed_list1))]
            self.pre_seed_list1 = seed_list1
            # print("pre_seed_list1:", self.pre_seed_list1)
            self.pre_seed_list2 = seed_list2
            # print("pre_seed_list2:", self.pre_seed_list2)
        ...
        # Start iteration
        # print("cea_mode:", self.cea_mode)

        for k in range(self.align_iteration):
            ...
            if not self.have_embed:
                ...
                if not self.cea_mode:
                    if iteration == 0:
                        ...
                        print(f'len emb1:{len(embedding1)}, shape emb1[0]:{embedding1[0].shape}, shape emb1[1]:{embedding1[1].shape}, shape emb1[2]:{embedding1[2].shape}')
                        print(f'len emb2:{len(embedding2)}, shape emb2[0]:{embedding2[0].shape}, shape emb2[1]:{embedding2[1].shape}, shape emb2[2]:{embedding2[2].shape}')
                ...
                embedding1, embedding2 = emb1, emb2
                embedding_fin1 = embedding1.detach().numpy()
        ...
        S_prime, result = self.FinalEvaluation(S, embedding1, embedding2, seed_list1, seed_list2, self.idx1_dict,
                                               self.idx2_dict, adj2)
        return embedding_fin1, embedding_fin2, seed_list1, seed_list2, S
    ...
    def AddSeeds(self, embedding1, embedding2, index, columns, seed_list1, seed_list2, iteration,
        ...
        if not have_embed:
            ...
            S_fin = torch.matmul(F.normalize(embedding1), F.normalize(embedding2).t())
    ...
    def Evaluation(self, seed_list1, seed_list2):
        ...
        train_len = int(self.seed_ratio * len(self.alignment_dict))

        # try:
        #     print("train_len:{}, seed_list1 len:{}".format(train_len, len(seed_list1)))
    ...
    def FinalEvaluation(self, S, embedding1, embedding2, seed_list1, seed_list2, idx1_dict, idx2_dict, adj2):
        ...
        result = '@1:' + str(round(top1_eval, 4)) + ',  @5:' + str(round(top5_eval, 4)) + ',  @10:' + str(
            round(top10_eval, 4)) + ',  Acc:' + str(round(acc, 4))
